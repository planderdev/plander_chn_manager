...
export default function ChipSelect({
  ...
  function handleRemove(id: number, value: string) {
    if (!confirm(t('chip.deleteConfirm', { value }))) return;
    startTransition(async () => {
          ...
          return (
            <div key={o.id}
              ...
              <button type="button" onClick={() => handleRemove(o.id, o.value)}
                ...
                title={t('chip.deleteTitle')}>×</button>
            </div>
          );
        })}
      ...
      {showInput && (
        <div className="flex gap-2 items-start">
          <input type="text" value={newValue}
            ...
            autoFocus placeholder={t('chip.placeholder')} disabled={pending}
            className="border border-gray-400 rounded p-2 text-sm flex-1" />
          <button type="button" onClick={handleAdd} disabled={pending}
            className="bg-black text-white px-3 py-2 rounded text-sm disabled:bg-gray-400">{t('common.save')}</button>
          <button type="button"
            onClick={() => { setShowInput(false); setNewValue(''); setError(''); }}
            className="border border-gray-400 px-3 py-2 rounded text-sm">{t('common.cancel')}</button>
        </div>
      )}
