...
export default async function PostsPage() {
  ...
  if (!hasSupabaseEnv()) {
    const posts = [
      { id: 1, clients: { company_name: 'Shanghai Bloom Clinic' }, influencers: { handle: 'linzi_daily' }, influencer_id: 1, post_url: 'https://www.xiaohongshu.com/explore/mock-1', uploaded_on: '2026-04-14', settlement_status: 'pending', settled_on: null },
      { id: 2, clients: { company_name: 'Suzhou Tea House' }, influencers: { handle: 'momo_foodnote' }, influencer_id: 2, post_url: null, uploaded_on: null, settlement_status: 'pending', settled_on: null },
      { id: 3, clients: { company_name: 'Hangzhou Skin Studio' }, influencers: { handle: 'xiao_homevibe' }, influencer_id: 3, post_url: 'https://www.douyin.com/video/mock', uploaded_on: '2026-04-10', settlement_status: 'done', settled_on: '2026-04-15' },
    ];
    return (
      <div className="p-4 md:p-8">
        ...
        <div className="flex items-center justify-between mb-6 flex-wrap gap-2"><h1 className="text-2xl font-bold">{t('posts.title')}</h1><div className="flex gap-2"><Link href="/influencers/posts/new" className="bg-black text-white px-4 py-2 rounded">{t('posts.new')}</Link></div></div>
        <div className="bg-white rounded-lg shadow overflow-x-auto"><table className="w-full text-sm min-w-[800px]"><thead className="bg-gray-100 text-left"><tr><th className="p-3">{t('common.companyName')}</th><th className="p-3">{t('common.influencer')}</th><th className="p-3">{t('common.post')}</th><th className="p-3">{t('postForm.uploadedOn')}</th><th className="p-3">{t('postForm.settlementStatus')}</th><th className="p-3">{t('postForm.settledOn')}</th><th className="p-3">{t('schedule.manage')}</th></tr></thead><tbody>{posts.map((p: any) => <tr key={p.id} className="border-t"><td className="p-3">{p.clients.company_name}</td><td className="p-3">@{p.influencers.handle}</td><td className="p-3">{p.post_url ? <a href={p.post_url} target="_blank" className="text-blue-600 hover:underline">{t('common.link')}</a> : <span className="text-gray-400">{t('posts.uploaded')}</span>}</td><td className="p-3">{p.uploaded_on ?? '-'}</td><td className="p-3"><span className={p.settlement_status === 'done' ? 'text-green-600' : 'text-orange-500'}>{p.settlement_status === 'done' ? t('postForm.done') : t('postForm.pending')}</span></td><td className="p-3">{p.settled_on ? p.settled_on.replaceAll('-', '/') : '-'}</td><td className="p-3 space-x-2"><span className="text-blue-600">{t('common.edit')}</span><span className="text-red-500">{t('common.delete')}</span></td></tr>)}</tbody></table></div>
      </div>
    );
  }
